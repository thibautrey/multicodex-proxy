...
const samples = Math.max(1, Number(samplesArgument?.slice("--samples=".length) ?? 2000));
const segmentsArgument = process.argv.find((value) => value.startsWith("--segments="));
const segments = Math.max(1, Number(segmentsArgument?.slice("--segments=".length) ?? 24));

process.env.MULTIVIBE_PROXY_CORE_NATIVE = mode === "native" ? "on" : "off";
